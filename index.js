...
if (userMessage.includes("朝")) {
  systemPrompt += " 朝のヨガを短いステップ形式で3～5分以内でできるように、数字や秒数も入れて説明してください。";
  systemPrompt += " さらに、この呼吸やポーズをしている他のYuj仲間を想像できるような一文も入れてください。";
} else if (userMessage.includes("夜")) {
  systemPrompt += " 夜のリラックスヨガを短いステップ形式で3～5分以内でできるように、数字や秒数も入れて説明してください。";
  systemPrompt += " 呼吸中に他のユーザーや大切な人を思い浮かべられる文も加えてください。";
} else if (userMessage.includes("肩")) {
  systemPrompt += " 肩こりに効果的なヨガポーズを短いステップ形式で3～5分以内でできるように、数字や秒数も入れて説明してください。";
  systemPrompt += " その中で他の仲間が一緒にやっているイメージも添えてください。";
} else if (userMessage.includes("ストレス")) {
  systemPrompt += " ストレスをやわらげる呼吸法やポーズを紹介してください。";
  systemPrompt += " その呼吸やポーズをしている仮想仲間を想像できる文を付けてください。";
} else {
  systemPrompt += " その内容に合わせた前向きなメッセージを短く伝えてください。";
  systemPrompt += " 読んだ人が『他の人も同じことをしている』と感じられる一文も追加してください。";
}
